...
app.post('/api/send-candidature', async (req, res) => {
  try {
    ...
    console.log('📧 Email empregador recebido da vaga:', employerEmail)
    ...
    // ===== MODO TESTE DO EMPREGADOR =====
    // Se TEST_EMPLOYER_EMAIL estiver configurado no .env,
    // todos os e-mails do empregador irão para esse endereço.
    // Para produção, deixe TEST_EMPLOYER_EMAIL vazio ou remova do .env.
    ...
    console.log('📧 E-mail do empregador recebido da vaga:', rawEmployerEmail || 'VAZIO')
    console.log('📧 E-mail do empregador usado no envio:', employerTargetEmail || 'NENHUM')
    console.log('🧪 Modo teste empregador:', isTestMode ? 'ATIVO' : 'DESATIVADO')

    if (
      employerTargetEmail &&
      employerTargetEmail !== 'Contato não informado' &&
      employerTargetEmail.includes('@')
    ) {
      console.log(`📤 Enviando e-mail para empregador: ${employerTargetEmail}`)
      ...
      console.warn('⚠️ E-mail do empregador inválido ou não informado. Envio para empregador ignorado.')
    }
