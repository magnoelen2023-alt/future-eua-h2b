...
export default function App() {
  ...
  const [selectedSeason, setSelectedSeason] = useState(saved.selectedSeason || 'winter-2025')
  const [sentLogs, setSentLogs] = useState(saved.sentLogs || [])
  ...
  // MODIFICAÇÃO AQUI: Simplificado para apenas currículo e carta
  const [registerForm, setRegisterForm] = useState({
    name: '', email: '', password: '', phone: '',
    address: '', cep: '', state: '', country: '',
    resumeFile: null,          // Substitui resume1, resume2, resume3
    coverLetterFile: null,     // Carta de apresentação
    resumeFileName: '',        // Apenas para exibir nome
    coverLetterFileName: '',   // Apenas para exibir nome
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ selectedSeason, sentLogs, queue }))
  }, [selectedSeason, sentLogs, queue])

  useEffect(() => {
    setSelectedIds([]); setSelectedJobId(null); setJobMessage(null)
  }, [selectedSeason])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // Prepara anexos para o email (URLs dos documentos do usuário)
      const attachments = []
  ...
  async function handleRegister(e) {
    ...
    try {
      // Upload dos arquivos antes de criar o usuário
      ...
      const newUser = {
        ...
        resume1_path: resumeUrl,           // Mantém compatibilidade com banco existente
        resume2_path: null,                // Campos vazios (não mais usados)
        resume3_path: null,
      ...
      if (String(error?.message || '').toLowerCase().includes('duplicate')) {
        ...
        message = 'Erro ao enviar arquivos. Verifique o tamanho (máx 5MB) e tente novamente.'
      }
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (error) {
        ...
        setLoginError('Erro ao consultar cadastro. Tente novamente.')
        return
      }

      if (!userData) {
        setLoginError('E-mail não encontrado. Verifique se digitou corretamente.')
        return
      }
  ...
  async function handleRecover(e) {
    ...
    try {
      ...
      if (error) {
        ...
        setRecoveryStatus({ type: 'error', text: 'Erro ao buscar cadastro.' })
        return
      }
      ...
      setRecoveryStatus({ type: 'success', text: `Sua senha cadastrada é: ${userData.password}` })
    } catch (err) {
      console.error(err)
      setRecoveryStatus({ type: 'error', text: 'Erro inesperado ao recuperar senha.' })
    }
  }

  // MODIFICADO: Função simplificada para registro (apenas currículo e carta)
  function handleRegisterFile(field, e) {
    ...
    if (registerStatus?.text?.includes('grande')) setRegisterStatus(null)
  }

  // MODIFICADO: Função simplificada para perfil (apenas currículo e carta)
  ...
  function openProfile() {
    ...
    setProfileForm({ 
      ...
      resumeFile: null,              // Novo arquivo selecionado
      coverLetterFile: null,         // Novo arquivo selecionado
  ...
  // MODIFICADO: saveProfile simplificado (apenas currículo e carta)
  async function saveProfile(e) {
    e.preventDefault()
    
    // Valida se há currículo
    ...
    try {
      ...
      // Só faz upload se houver novo arquivo OU não existir URL antiga
      if (profileForm.resumeFile || !user?.resume1_path) {
      ...
      if (profileForm.coverLetterFile || (!user?.cover_letter_path && profileForm.coverLetterFile)) {
        console.log('📤 Processando Carta...');
  ...
  async function activatePremiumKey() {
    ...
    try {
      ...
      if (!data.ok) { setActivationStatus({ type: 'error', text: data.error || 'Chave inválida.' }); return }
      ...
      setActivationStatus({ type: 'success', text: '✅ Chave Premium ativada! Seus dados foram protegidos.' })
      setActivationKey('')
    } catch {
      setActivationStatus({ type: 'error', text: 'Erro de conexão. Verifique se o backend está rodando.' })
    }
  }
      ...
      {page === 'register' && (
        <AuthShell>
          <form className="auth-card large" onSubmit={handleRegister}>
            ...
            {uploadingFiles && <div className="alert info">⏳ Enviando arquivos para o servidor...</div>}
            ...
            {/* MODIFICADO: Seção Documentos simplificada (apenas currículo e carta) */}
            <div className="form-section">
              <h3>Documentos</h3>
              <div className="grid two">
                <label>Currículo Principal *
                  <input 
                    type="file" 
                    accept=".pdf,.doc,.docx" 
                    onChange={e => handleRegisterFile('resume', e)} 
                ...
                <label>Carta de Apresentação (Opcional)
                  <input 
                    type="file" 
                    accept=".pdf,.doc,.docx" 
                    onChange={e => handleRegisterFile('coverLetter', e)} 
                    disabled={uploadingFiles}
                  />
              ...
              <small style={{color: '#666', display: 'block', marginTop: '8px'}}>
                Máximo 5MB por arquivo. Formatos aceitos: PDF, DOC, DOCX
              </small>
            </div>
            ...
            <button className="primary-btn" type="submit" disabled={uploadingFiles}>
              {uploadingFiles ? '⏳ Enviando...' : 'Concluir cadastro'}
            </button>
          ...
          <form className="auth-card large" onSubmit={saveProfile}>
            ...
            {isPremium && (
              <div className="alert success">
                ✅ <strong>Conta Premium Ativa.</strong> Nome, e-mail e endereço estão protegidos.
              </div>
            )}
            ...
            <div className="form-section">
              <h3>Dados pessoais</h3>
              <div className="grid two">
                <Field label="Nome completo" value={profileForm.name || ''} disabled={isPremium} onChange={v => setProfileForm(p => ({ ...p, name: v }))} />
                <Field label="E-mail (não editável)" value={profileForm.email || ''} disabled={true} onChange={() => {}} />
                <Field label="Telefone" value={profileForm.phone || ''} onChange={v => setProfileForm(p => ({ ...p, phone: v }))} />
                <Field label="Endereço" value={profileForm.address || ''} disabled={isPremium} onChange={v => setProfileForm(p => ({ ...p, address: v }))} />
                <Field label="CEP" value={profileForm.cep || ''} disabled={isPremium} onChange={v => setProfileForm(p => ({ ...p, cep: v }))} />
                <Field label="Estado" value={profileForm.state || ''} disabled={isPremium} onChange={v => setProfileForm(p => ({ ...p, state: v }))} />
                <Field label="País" value={profileForm.country || ''} disabled={isPremium} onChange={v => setProfileForm(p => ({ ...p, country: v }))} />
              </div>
            </div>
              ...
              <div className="grid two">
                <label>Currículo Principal
                  <input 
                    type="file" 
                    accept=".pdf,.doc,.docx" 
                    onChange={e => handleProfileFile('resume', e)} 
                    disabled={uploadingFiles}
                  />
                  {profileForm.resumeFileName && <small>✅ {profileForm.resumeFileName}</small>}
                  {user?.resume1_path && <small style={{color: '#666'}}>
                    🔗 <a href={user.resume1_path} target="_blank" rel="noreferrer">Ver atual</a>
                  </small>}
                </label>
                ...
                <label>Carta de Apresentação (Opcional)
                  <input 
                    type="file" 
                    accept=".pdf,.doc,.docx" 
                    onChange={e => handleProfileFile('coverLetter', e)} 
                    disabled={uploadingFiles}
                  />
                  {profileForm.coverLetterFileName && <small>✅ {profileForm.coverLetterFileName}</small>}
                  {user?.cover_letter_path && <small style={{color: '#666'}}>
                    🔗 <a href={user.cover_letter_path} target="_blank" rel="noreferrer">Ver atual</a>
                  </small>}
                </label>
              </div>
              <small style={{color: '#666', display: 'block', marginTop: '8px'}}>
                Selecione novos arquivos apenas se quiser substituir os documentos atuais. Máximo 5MB por arquivo.
              </small>
            </div>
            ...
            {!isPremium && (
              <div className="form-section premium-activation-box">
                ...
                <p className="muted">Após comprar sua chave, cole-a abaixo para liberar o acesso completo (100 envios/dia).</p>
                ...
                <input
                  ...
                  onChange={e => setActivationKey(
                    e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/(.{4})/g, '$1-').replace(/-$/, '').slice(0, 19)
                  )}
                />
                <button type="button" className="primary-btn" style={{ marginTop: 12, width: '100%' }} onClick={activatePremiumKey}>
                  Ativar Premium
                </button>
                <p style={{ marginTop: 10, fontSize: 13, opacity: 0.7, textAlign: 'center' }}>
                  Não tem uma chave?{' '}
                  <a href={CONTACT_LINK} target="_blank" rel="noreferrer" style={{ color: '#facc15' }}>
            ...
            {isPremium && (
              <div className="alert success" style={{ marginTop: 16 }}>
                ✅ Chave Premium ativa. Validade:{' '}
                {user?.premiumExpiresAt ? new Date(user.premiumExpiresAt).toLocaleDateString('pt-BR') : 'Indeterminada'}
              </div>
            )}
            <button className="primary-btn" type="submit" style={{ marginTop: 16 }} disabled={uploadingFiles}>
              {uploadingFiles ? '⏳ Salvando...' : 'Salvar perfil'}
            </button>
...
// ... RESTANTE DO CÓDIGO (componentes Home, AuthShell, BrandBlock, RotatingLogo, Field, Dashboard, JobsPage, TopBar, Avatar, StatCard, InfoLine, GlobalFooter) ...
...
// ... Continuação dos componentes Dashboard, JobsPage, TopBar, Avatar, StatCard, InfoLine, GlobalFooter ...
// (EXATAMENTE COMO NO SEU CÓDIGO ORIGINAL)

function Dashboard({
  user, currentSeason, selectedSeason, setSelectedSeason,
  sentCount, remainingCount, progress, barColor, todaySent, averageDaily,
  systemStatus, finalBlocked, dailyBlocked, todayQueued, dailyRemaining,
  queueLength, activeSend, countdown, onJobs, onProfile, onLogout,
  totalSeasonJobs, loadingJobs, isPremium, totalSentEver, isDemoBlocked,
}) {
  return (
    <main className="dashboard-page">
      ...
      <section className="container">
        <div className="dashboard-hero">
          <div>
            <span className="pill">Painel principal</span>
            <h2>Dashboard da temporada</h2>
            <p>Acompanhe progresso geral e status do sistema.</p>
          </div>
        ...
        <div className="key-card">
          <div>
            <strong>{isPremium ? '✅ Conta Premium Ativa' : '🔑 Acesso Demonstração'}</strong>
            <p>Chave: <span>{user?.access_key || user?.accessKey || FREE_ACCESS_KEY}</span></p>
            {!isPremium && (
              <p style={{ color: '#fbbf24', marginTop: 6 }}>
                Envios de teste: <strong>{totalSentEver}</strong> / {DEMO_LIMIT}
              </p>
            )}
            {isPremium && user?.premiumExpiresAt && (
              <p style={{ color: '#86efac', marginTop: 6 }}>
        ...
        {!isPremium && totalSentEver >= DEMO_LIMIT && (
          <div className="alert error demo-lock-box">
            <p>🚀 <strong>Período de teste finalizado</strong></p>
            <p>Você utilizou {totalSentEver}/{DEMO_LIMIT} envios gratuitos.</p>
            <p>Para continuar, adquira sua Chave Premium ou ative-a em <strong>Editar Perfil</strong>.</p>
            <a href={CONTACT_LINK} target="_blank" rel="noreferrer" className="buy-key-btn">Comprar Chave Premium</a>
          </div>
        )}

        <div className="stats-grid">
          <StatCard title="Total da temporada" value={`${totalSeasonJobs} vagas`} />
          <StatCard title="Enviadas" value={sentCount} />
          <StatCard title="Restantes" value={remainingCount} />
          <StatCard title="Status" value={systemStatus} />
        </div>

        <section className="panel">
          <div className="panel-head">
            <div><h3>Progresso geral</h3><p>{currentSeason?.label}</p></div>
            <strong>{progress}%</strong>
          </div>
          <div className="life-bar">
            <div className={`life-fill ${barColor}`} style={{ width: `${progress}%` }} />
          </div>
          <div className="progress-info">
            <span>Verde: início</span>
            <span>Amarelo: médio</span>
            <span>Vermelho: quase final</span>
          </div>
        </section>

        <div className="stats-grid">
          <StatCard title="Hoje" value={`${todaySent} enviadas`} />
          <StatCard title="Na fila hoje" value={todayQueued} />
          <StatCard title="Média diária" value={averageDaily} />
        ...
        <section className="panel">
          ...
          <div className="queue-info">
            <div><strong>{queueLength}</strong><span>itens na fila</span></div>
            <div><strong>{dailyRemaining}</strong><span>envios restantes</span></div>
            <div><strong>{activeSend ? formatSeconds(countdown) : '—'}</strong><span>próximo envio</span></div>
          </div>
        </section>

        <div className="actions-row">
          {!finalBlocked && !isDemoBlocked && (
            <button className="primary-btn" onClick={onJobs}>Abrir painel de vagas</button>
          )}
          {isDemoBlocked && (
            <button className="primary-btn" onClick={onJobs}>Ver vagas</button>
          )}
...
function JobsPage({
  user, currentSeason, selectedSeason, setSelectedSeason,
  visibleJobs, filteredJobs, selectedJob, setSelectedJobId,
  selectedIds, toggleSelect, selectFirstAvailable, clearSelection, sendSelected,
  sentIds, queuedIds, search, setSearch, categoryFilter, setCategoryFilter,
  stateFilter, setStateFilter, states, categories, finalBlocked, dailyBlocked,
  dailyRemaining, todaySent, todayQueued, jobMessage, queueRunning, setQueueRunning,
  activeSend, countdown, fastMode, setFastMode, onDashboard, onProfile,
  loadingJobs, isDemoBlocked, totalSentEver, isPremium,
}) {
  ...
  return (
    <main className="jobs-page">
      ...
      <section className="container">
        <div className="dashboard-hero jobs-hero-custom">
          <div>
            <span className="pill">Painel de vagas</span>
            <h2>Painel de vagas</h2>
            <p>
              {isPremium
                ? `Limite diário: 100 vagas. Restantes hoje: ${dailyRemaining}`
                : `Modo demonstração: ${totalSentEver}/${DEMO_LIMIT} envios usados.`}
            </p>
          </div>
          <div className="season-box">
            <label>Temporada</label>
        ...
        <div className="mini-status">
          <span>Temporada: {currentSeason?.label}</span>
          <span>Hoje: {todaySent} enviados</span>
          <span>Fila hoje: {todayQueued}</span>
        ...
        {isDemoBlocked && (
          <div className="alert error demo-lock-box">
            <p>🚀 <strong>Período de teste finalizado</strong></p>
            <p>Você utilizou {totalSentEver}/{DEMO_LIMIT} envios gratuitos. Para continuar, ative sua Chave Premium em <strong>Editar Perfil</strong>.</p>
            <a href={CONTACT_LINK} target="_blank" rel="noreferrer" className="buy-key-btn">Comprar Chave Premium</a>
          </div>
        )}

        {!isDemoBlocked && sendHidden && (
          <div className="alert error">
            {finalBlocked ? 'Temporada finalizada.' : 'Limite diário atingido.'}
          </div>
        )}

        <div className="jobs-top-filters">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔎 Pesquisar vaga (ex.: camareira, dishwasher, Ohio...)" />
        ...
        <div className="jobs-total-line">
          {!loadingJobs && <span>{filteredJobs.length} vaga(s) no filtro.</span>}
        </div>

        <div className="jobs-selection-summary">
          <div><strong>{selectedIds.length} vaga(s) selecionada(s)</strong></div>
          <div className="jobs-selection-actions">
            <button className="ghost-btn" onClick={selectFirstAvailable} disabled={sendHidden || loadingJobs || isDemoBlocked}>Selecionar disponíveis</button>
            <button className="ghost-btn" onClick={clearSelection}>Limpar</button>
            <button
              className={`primary-btn ${isDemoBlocked || sendHidden ? 'disabled-off' : ''}`}
              onClick={sendSelected}
              disabled={selectedIds.length === 0 || loadingJobs || sendHidden || isDemoBlocked}
            >
              {isDemoBlocked ? 'Teste finalizado' : `Enviar (${selectedIds.length})`}
            </button>
          </div>
        </div>

        <div className="jobs-queue-bar">
          <label className="check-row"><input type="checkbox" checked={queueRunning} onChange={e => setQueueRunning(e.target.checked)} /> Processar fila</label>
          <label className="check-row"><input type="checkbox" checked={fastMode} onChange={e => setFastMode(e.target.checked)} /> Modo teste rápido</label>
          <span className="queue-next-send">Próximo envio: <strong>{activeSend ? formatSeconds(countdown) : 'aguardando'}</strong></span>
        </div>
        ...
        <div className="jobs-two-columns">
          <section className="jobs-list-panel">
            <div className="jobs-panel-head">
              ...
              <p>{loadingJobs ? 'Carregando...' : `Mostrando ${visibleJobs.length} de ${filteredJobs.length}`}</p>
            </div>
            <div className="jobs-list-scroll">
              {!loadingJobs && visibleJobs.map(job => {
                const alreadySent = sentIds.has(job.id)
                const alreadyQueued = queuedIds.has(job.id)
                const selected = selectedIds.includes(job.id)
                return (
                  <div key={job.id} className={`job-master-item ${selectedJob?.id === job.id ? 'active' : ''}`}>
                    <button
                      type="button"
                      className={`job-select-dot ${selected ? 'checked' : ''} ${alreadySent ? 'sent' : ''} ${alreadyQueued ? 'queued' : ''}`}
                      disabled={sendHidden || alreadySent || alreadyQueued || isDemoBlocked || (!selected && selectedIds.length >= dailyRemaining)}
                    ...
                    <button type="button" className="job-master-main" onClick={() => setSelectedJobId(job.id)}>
                      ...
                      <div className="job-master-subline">
                        <span>{job.employer}</span>
                        {alreadySent && <span className="job-status-pill sent">Enviado</span>}
                        {alreadyQueued && <span className="job-status-pill queued">Na fila</span>}
                      </div>
                    </button>
                    <div className="job-master-right"><strong>{job.wage}</strong></div>
                  </div>
                )
              })}
              {!loadingJobs && visibleJobs.length === 0 && <div className="empty-box">Nenhuma vaga encontrada.</div>}
            </div>
          </section>

          <section className="jobs-detail-panel">
            <div className="jobs-panel-head">
              <h3>DETALHE DA VAGA</h3>
              <p>{selectedJob ? selectedJob.category : 'Selecione uma vaga'}</p>
            </div>
            <div className="jobs-detail-scroll">
              {selectedJob ? (
                <div className="detail-card detail-card-pt">
                  ...
                  <InfoLine label="Local" value={selectedJob.fullLocation} />
                  <InfoLine label="Cidade / Estado" value={`${selectedJob.city} / ${selectedJob.state}`} />
                  <InfoLine label="Vagas" value={`${selectedJob.available}`} />
                  <InfoLine label="Início" value={selectedJob.startDate} />
                  <InfoLine label="Fim" value={selectedJob.endDate} />
                  <InfoLine label="Salário" value={selectedJob.wageDetail} />
                  <InfoLine label="Telefone" value={selectedJob.phone || 'Não informado'} />
                  <InfoLine label="E-mail empregador" value={selectedJob.contact} />
                  <InfoLine label="Case Number" value={selectedJob.caseNumber} />
                  <InfoLine label="Tipo do visto" value={selectedJob.visaType} />
                  <div className="description-box">
                    <strong>Descrição da vaga</strong>
                    <p>{buildPortugueseJobDescription(selectedJob)}</p>
                  </div>
                  <div className="message-preview">
                    <strong>Mensagem padrão do candidato</strong>
                    <p>{user?.employer_message || user?.employerMessage}</p>
                  </div>
                </div>
              ) : (
                <div className="empty-box">Selecione uma vaga para ver os detalhes.</div>
              )}
            </div>
          </section>
        </div>
  ...
  return (
    <header className="topbar">
      <div className="topbar-brand">
        <RotatingLogo />
        <div><strong>FUTURE EUA H2B</strong><span>Rumo ao sonho americano</span></div>
      </div>
      <nav>
        <button onClick={onDashboard}>Dashboard</button>
        {!finalBlocked && <button onClick={onJobs}>Vagas</button>}
        <button onClick={onProfile}>Editar perfil</button>
      </nav>
      <div className="user-box">
        <Avatar user={user} />
        <div><strong>{user?.name}</strong><span>{user?.email}</span></div>
        {!hideLogout && <button className="logout-btn" onClick={onLogout}>Sair</button>}
      </div>
    </header>
  )
}
  ...
  return (
    <footer className="global-footer">
      ...
      <div style={{ marginTop: '16px', textAlign: 'center' }}>
        <p style={{ fontWeight: 'bold', fontSize: '15px', color: '#facc15', marginBottom: '8px' }}>
          © {new Date().getFullYear()} Bymagno_rust — Todos os direitos reservados
        </p>
        <p style={{ fontSize: '13px', opacity: 0.85, marginBottom: '6px' }}>
          📧 Contato: <a href="mailto:magno.elen2023@gmail.com" style={{ color: '#60a5fa', textDecoration: 'none' }}>magno.elen2023@gmail.com</a>
        </p>
        <p style={{ fontSize: '13px', opacity: 0.85 }}>
          <a 
            href="https://wa.me/5575999866105?text=Olá!%20Tenho%20interesse%20no%20FUTURE%20EUA%20H2B" 
            target="_blank" 
            rel="noreferrer"
            style={{ 
              color: '#25D366', 
