...
function parseJobFromCsv(row, index, seasonId) {
  ...
  const agentAttorneyName = getRowValue(row, ['Agent Attorney Name', 'Nome do advogado do agente', 'agent_attorney_name'])
  const state = formatState(getRowValue(row, ['Worksite State', 'Estado', 'state']))
  const randomizationGroup = getRowValue(row, ['Randomization Group', 'randomization_group'])
  const startDate = formatDate(getRowValue(row, ['Begin Date', 'Data de início', 'Data de inicio', 'begin_date']))
  const endDate = formatDate(getRowValue(row, ['Final Date', 'End Date', 'Data de fim', 'end_date']))
  const contact = getRowValue(row, ['Email', 'E-mail do empregador', 'Email do empregador', 'contact'])
  ...
  const descriptionEn = cleanLongText(getRowValue(row, ['Descricao_Vaga', 'Descrição_Vaga', 'Description', 'description', 'Responsabilidades do cargo', 'Responsabilidade do cargo']))
  ...
  const finalDescription = descriptionPt ? descriptionPt : translateDescriptionToPt(descriptionEn || 'Descrição não informada.')
...
function getLicenseKey(user = {}) {
  return user?.access_key || user?.accessKey || user?.premium_access_key || user?.premiumAccessKey || ''
}
...
export default function App() {
  ...
  const loadFromSupabase = useCallback(async (userId) => {
    ...
    try {
      ...
      if (data) {
        const normalizedSentLogs = Array.isArray(data.sent_logs) ? data.sent_logs.map(l => l?.seasonId === 'winter-2025' ? { ...l, seasonId: 'winter-2026' } : l) : []
        const normalizedQueue = Array.isArray(data.queue_data) ? data.queue_data.map(i => i?.seasonId === 'winter-2025' ? { ...i, seasonId: 'winter-2026' } : i) : []
        const normalizedData = { ...data, sent_logs: normalizedSentLogs, queue_data: normalizedQueue, selected_season: normalizeSeasonId(data.selected_season) }
  ...
  const saveToSupabase = useCallback(async (userId, newSentLogs, newQueue, newSeason) => {
    ...
    try {
      await supabase.from('users').update({ sent_logs: newSentLogs, queue_data: newQueue, selected_season: normalizeSeasonId(newSeason) }).eq('id', userId)
  ...
  useEffect(() => {
    ...
    if (status === 'sucesso') {
      ...
      alert('❌ Erro ao integrar com o Gmail.')
      window.history.replaceState({}, document.title, window.location.pathname)
    }
  ...
  useEffect(() => {
    function handleVisibility() {
      if (document.visibilityState === 'visible' && user?.id) loadFromSupabase(user.id)
    }
  ...
  // ===================== ALTERAÇÃO PRINCIPAL =====================
  const unsentJobs = useMemo(() => {
    return filteredJobs.filter(job => !sentIds.has(job.id) && !queuedIds.has(job.id))
  }, [filteredJobs, sentIds, queuedIds])

  const visibleJobs = unsentJobs.slice(0, 500)
  // ============================================================
    ...
    const timer = setTimeout(() => {
      ;(async () => {
        ...
        try {
          ...
          const newLog = { id: createSentId(), jobId: item.jobId, jobTitle: item.jobTitle, employer: item.employer, contact: item.contact, seasonId: item.seasonId, sentAt: new Date().toISOString() }
          ...
          if (user?.id && dataLoadedRef.current) saveToSupabase(user.id, newSentLogs, newQueue, selectedSeason)
        } catch (err) {
  ...
  async function uploadFileToStorage(file, folder = 'documents') {
    ...
    try {
      ...
      return publicUrl
    } catch (error) { throw new Error(`Falha ao enviar arquivo: ${error.message}`) }
  }
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      const newUser = {
        ...
        phone: registerForm.phone.trim(), address: registerForm.address, cep: registerForm.cep, state: registerForm.state,
        country: registerForm.country || 'Brasil', employer_message: registerForm.employerMessage, premium: false,
        ...
        sent_logs: [], queue_data: [], selected_season: 'winter-2026',
      }
      ...
      setRegisterStatus({ type: 'success', text: '✅ Cadastro realizado com sucesso!' })
  ...
  async function handleRecover(e) {
    ...
    } catch { setRecoveryStatus({ type: 'error', text: 'Erro ao recuperar senha.' }) }
  }
  ...
  function openProfile() {
    if (!user) return
    setProfileForm({ ...user, employerMessage: user.employer_message || '', resumeFile: null, coverLetterFile: null, resumeFileName: user.resume1_path ? 'Arquivo carregado' : '', coverLetterFileName: user.cover_letter_path ? 'Arquivo carregado' : '' })
  ...
  const handleConnectGmail = async () => {
    ...
    const activeApiUrl = window.location.hostname.includes('vercel.app') ? 'https://future-eua-h2b-api.onrender.com' : API_URL
    try {
      const res = await fetch(`${activeApiUrl}/api/gmail/auth-url?email=${encodeURIComponent(user.email)}`)
      if (!res.ok) throw new Error(`HTTP Error: ${res.status}`)
      const data = await res.json()
      if (data.url) window.location.href = data.url
      else alert('Erro ao gerar link do Google.')
    } catch { alert('Erro de conexão com o servidor de autenticação.') }
  ...
  async function activatePremiumKey() {
    ...
    try {
      const response = await fetch(`${API_URL}/api/activate-key`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: cleanedKey, user }) })
  ...
  function sendSelected() {
    ...
    const newItems = selJobs.map(job => ({ id: createQueueId(), jobId: job.id, jobTitle: job.title, employer: job.employer, contact: job.contact, seasonId: selectedSeason, createdAt: new Date().toISOString(), status: 'queued' }))
  ...
  return (
    <div className="app">
      ...
      {page === 'dashboard' && (
        <Dashboard
          user={user} currentSeason={currentSeason} selectedSeason={selectedSeason} setSelectedSeason={setSelectedSeason}
          sentCount={sentCount} remainingCount={remainingCount} progress={progress} barColor={barColor} todaySent={todaySent}
          averageDaily={todaySent} systemStatus={systemStatus} finalBlocked={finalBlocked} dailyBlocked={dailyBlocked}
          todayQueued={todayQueued} dailyRemaining={dailyRemaining} queueLength={queue.length} activeSend={activeSend}
          countdown={countdown} onJobs={() => requireLogin('jobs')} onProfile={openProfile} onLogout={handleLogout}
          totalSeasonJobs={totalSeasonJobs} loadingJobs={loadingJobs} isPremium={isPremium} totalSentEver={totalSentEver}
      ...
      {page === 'jobs' && (
        <JobsPage
          user={user} currentSeason={currentSeason} selectedSeason={selectedSeason} setSelectedSeason={setSelectedSeason}
          visibleJobs={visibleJobs} filteredJobs={filteredJobs} selectedJob={selectedJob} setSelectedJobId={setSelectedJobId}
          selectedIds={selectedIds} toggleSelect={toggleSelect} selectFirstAvailable={selectFirstAvailable}
          clearSelection={clearSelection} sendSelected={sendSelected} sentIds={sentIds} queuedIds={queuedIds}
          search={search} setSearch={setSearch} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter}
          stateFilter={stateFilter} setStateFilter={setStateFilter} states={states} categories={categories}
          finalBlocked={finalBlocked} dailyBlocked={dailyBlocked} dailyRemaining={dailyRemaining} todaySent={todaySent}
          todayQueued={todayQueued} jobMessage={jobMessage} queueRunning={queueRunning} setQueueRunning={setQueueRunning}
        ...
        <AuthShell>
          <form className="auth-card large" onSubmit={saveProfile}>
            ...
            <div className="form-section gmail-integration-box" style={{ background: 'rgba(26, 58, 143, 0.1)', padding: '20px', borderRadius: '12px', border: '1px solid rgba(26, 58, 143, 0.3)', marginTop: '20px' }}>
              ...
              <p style={{ fontSize: '13px', opacity: 0.8, marginBottom: '15px', lineHeight: '1.4' }}>Conecte seu Gmail para enviar candidaturas pelo seu próprio e-mail e receber as respostas automáticas dos empregadores na sua caixa de entrada.</p>
              {gmailConnected ? (
                ...
                <button type="button" className="primary-btn" style={{ width: '100%', background: '#fff', color: '#000' }} onClick={handleConnectGmail} disabled={loadingGmail}>{loadingGmail ? '⏳ Carregando...' : '🔗 Conectar meu Gmail'}</button>
              )}
            </div>
            {!isPremium && (
    ...
    <main className="dashboard-page">
      ...
      <section className="container">
        ...
        <section className="panel" style={{ marginTop: '20px', border: '1px solid rgba(26, 58, 143, 0.3)', background: 'rgba(26, 58, 143, 0.05)' }}>
          <div className="panel-head"><div><h3>📧 Conexão Gmail</h3><p>Para receber respostas automáticas dos empregadores</p></div><span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', background: gmailConnected ? 'rgba(22, 163, 74, 0.2)' : 'rgba(239, 68, 68, 0.2)', color: gmailConnected ? '#4ade80' : '#f87171' }}>{gmailConnected ? 'CONECTADO' : 'DESCONECTADO'}</span></div>
          <div style={{ padding: '15px 0' }}>{gmailConnected ? (<div style={{ background: 'rgba(255,255,255,0.02)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}><p style={{ fontSize: '13px', color: '#ccc' }}>✅ Seu sistema está enviando pelo e-mail:</p><p style={{ fontSize: '14px', fontWeight: 'bold', marginTop: '5px', color: '#4ade80' }}>{gmailEmail || user?.email}</p></div>) : (<button type="button" className="primary-btn" style={{ width: '100%', background: '#fff', color: '#000' }} onClick={handleConnectGmail} disabled={loadingGmail}>{loadingGmail ? '⏳ Redirecionando...' : '🔗 Conectar meu Gmail agora'}</button>)}</div>
        </section>
    ...
    <main className="jobs-page">
      ...
      <section className="container">
        ...
        <div className="jobs-two-columns">
          <section className="jobs-list-panel"><div className="jobs-panel-head"><h3>LISTA DE VAGAS</h3><p>{loadingJobs ? 'Carregando...' : `Mostrando ${visibleJobs.length} de ${filteredJobs.length}`}</p></div><div className="jobs-list-scroll">{!loadingJobs && visibleJobs.map(job => { const alreadySent = sentIds.has(job.id); const alreadyQueued = queuedIds.has(job.id); const selected = selectedIds.includes(job.id); return (<div key={job.id} className={`job-master-item ${selectedJob?.id === job.id ? 'active' : ''}`}><button type="button" className={`job-select-dot ${selected ? 'checked' : ''} ${alreadySent ? 'sent' : ''} ${alreadyQueued ? 'queued' : ''}`} disabled={sendHidden || alreadySent || alreadyQueued || isDemoBlocked} onClick={e => { e.stopPropagation(); toggleSelect(job.id) }}><span /></button><button type="button" className="job-master-main" onClick={() => setSelectedJobId(job.id)}><div className="job-master-title-line"><strong>{job.title}</strong><span className="job-master-location">— {job.location}</span></div><div className="job-master-subline"><span>{job.employer}</span>{alreadySent && <span className="job-status-pill sent">Enviado</span>}{alreadyQueued && <span className="job-status-pill queued">Na fila</span>}</div></button><div className="job-master-right"><strong>{job.wage}</strong><small>{job.available || 0} vaga(s)</small></div></div>) })}{!loadingJobs && visibleJobs.length === 0 && <div className="empty-box">Nenhuma vaga encontrada.</div>}</div></section>
          <section className="jobs-detail-panel"><div className="jobs-panel-head"><h3>DETALHE DA VAGA</h3><p>{selectedJob ? selectedJob.category : 'Selecione uma vaga'}</p></div><div className="jobs-detail-scroll">{selectedJob ? (<div className="detail-card detail-card-pt"><h2>{selectedJob.title}</h2><p className="detail-employer">{selectedJob.employer}</p><InfoLine label="Cargo" value={selectedJob.title || 'Não informado'} /><InfoLine label="Case Number" value={selectedJob.caseNumber || 'Não informado'} /><InfoLine label="Empresa" value={selectedJob.employer || 'Não informado'} /><InfoLine label="Agente / Advogado" value={selectedJob.agentAttorneyName || 'Não informado'} /><InfoLine label="Estado" value={selectedJob.state || 'Não informado'} /><InfoLine label="Grupo de randomização" value={selectedJob.randomizationGroup || 'Não informado'} /><InfoLine label="Quantidade de vagas" value={`${selectedJob.available || 0}`} /><InfoLine label="Início" value={selectedJob.startDate || 'Não informado'} /><InfoLine label="Fim" value={selectedJob.endDate || 'Não informado'} /><InfoLine label="Valor por hora" value={selectedJob.wage || 'Não informado'} /><InfoLine label="Telefone" value={selectedJob.phone || 'Não informado'} /><InfoLine label="E-mail empregador" value={selectedJob.contact || 'Não informado'} /><InfoLine label="Tipo do visto" value={selectedJob.visaType || 'H-2B'} /><div className="description-box"><strong>Descrição oficial</strong><p>{selectedJob.description || 'Descrição não informada.'}</p></div><div className="message-preview"><strong>Mensagem padrão do candidato</strong><p>{user?.employer_message || user?.employerMessage}</p></div></div>) : (<div className="empty-box">Selecione uma vaga para ver os detalhes.</div>)}</div></section>
        </div>
      </section>
    </main>
  )
