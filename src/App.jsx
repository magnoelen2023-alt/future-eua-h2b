...
// Garantindo que aponte para o backend correto no Render se a Vercel não tiver a variável
...
function formatWageValue(value = '') {
  ...
  if (clean.includes('$') || clean.includes('US$')) return clean
  return `US$ ${clean}/h`
}
...
function createSentId() {
  return `SENT-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
}
...
export default function App() {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          queue_data: newQueue,
          selected_season: normalizeSeasonId(newSeason),
        })
        .eq('id', userId)
      ...
      for (const season of seasons) {
        ...
        try {
          ...
          const rows = (parsed.data || []).filter(r =>
            getRowValue(r, ['Case Number', 'Case number', 'case_number'])
          )
  ...
  const isPremium =
    user?.premium === true &&
    (!user?.premiumExpiresAt && !user?.premium_expires_at ||
      new Date(user?.premiumExpiresAt || user?.premium_expires_at) > new Date())
  ...
  const systemStatus = finalBlocked
  ...
  useEffect(() => {
    ...
    setQueue(prev =>
      prev.map(i => i.id === next.id ? { ...i, status: 'waiting', dueAt } : i)
    )
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ;(async () => {
        ...
        try {
          // O SEGREDO ESTÁ AQUI: ADICIONAMOS O userId AO BODY
          const response = await fetch(`${API_URL}/api/send-candidature`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              userId: user?.id, // <-- ESSA É A CHAVE PARA O GMAIL FUNCIONAR
          ...
          setJobMessage({
            type: 'error',
            text: `Falha ao enviar "${item.jobTitle}": ${err.message}`,
          })
        } finally {
          setActiveSend(null)
        }
  ...
  useEffect(() => {
    ...
    const update = () => {
      setCountdown(Math.max(0, Math.ceil((activeSend.dueAt - Date.now()) / 1000)))
    }
  ...
  async function activatePremiumKey() {
    ...
    if (!cleanedKey || cleanedKey.length < 10) {
      setActivationStatus({ type: 'error', text: 'Digite sua chave Premium completa.' })
      return
    }
  ...
  function toggleSelect(jobId) {
    ...
    if (sentIds.has(jobId) || queuedIds.has(jobId)) {
      setJobMessage({ type: 'error', text: 'Vaga já enviada ou na fila.' })
      return
    }

    if (selectedIds.includes(jobId)) {
      setSelectedIds(p => p.filter(id => id !== jobId))
      return
    }

    if (selectedIds.length >= dailyRemaining) {
      setJobMessage({ type: 'error', text: `Limite atingido. Restam ${dailyRemaining}.` })
      return
    }
  ...
  function selectFirstAvailable() {
    ...
    const avail = visibleJobs
      .filter(j => !sentIds.has(j.id) && !queuedIds.has(j.id))
        ...
        <AuthShell>
          <form className="auth-card large" onSubmit={handleRegister}>
            ...
            <div className="form-section">
              ...
              <label>
                <textarea rows="5" value={registerForm.employerMessage} onChange={e => setRegisterForm(p => ({ ...p, employerMessage: e.target.value }))} />
              </label>
            </div>
        ...
        <AuthShell>
          <form className="auth-card large" onSubmit={saveProfile}>
            ...
            <div className="form-section">
              <h3>Mensagem padrão para empregador</h3>
              <label>
                <textarea rows="6" value={profileForm.employerMessage || ''} onChange={e => setProfileForm(p => ({ ...p, employerMessage: e.target.value }))} />
              </label>
            </div>
            ...
            <div className="form-section gmail-integration-box" style={{ background: 'rgba(26, 58, 143, 0.1)', padding: '20px', borderRadius: '12px', border: '1px solid rgba(26, 58, 143, 0.3)', marginTop: '20px' }}>
              ...
              <p style={{ fontSize: '13px', opacity: 0.8, marginBottom: '15px', lineHeight: '1.4' }}>
                Conecte seu Gmail para enviar candidaturas pelo seu próprio e-mail e receber as respostas automáticas dos empregadores na sua caixa de entrada.
              </p>
              ...
              {gmailConnected ? (
                ...
                <button type="button" className="primary-btn" style={{ width: '100%', background: '#fff', color: '#000' }} onClick={handleConnectGmail} disabled={loadingGmail}>
...
function RotatingLogo() {
  return <div className="rotating-logo logo-image"><img src="/logo-br-us.png" alt="Future EUA H2B" /></div>
}
      ...
      <section className="container">
        ...
        <div className="key-card">
          <div>
            ...
            <p>Chave: <span>{user?.access_key || FREE_ACCESS_KEY}</span></p>
            {!isPremium && <p style={{ color: '#fbbf24', marginTop: 6 }}>Envios de teste: <strong>{totalSentEver}</strong> / {DEMO_LIMIT}</p>}
          </div>
      ...
      <section className="container">
        <div className="dashboard-hero jobs-hero-custom">
          <div>
            ...
            <p>{isPremium ? `Limite diário: 100 vagas. Restantes hoje: ${dailyRemaining}` : `Modo demonstração: ${totalSentEver}/${DEMO_LIMIT} envios usados.`}</p>
          </div>
        ...
        {!isDemoBlocked && sendHidden && (
          <div className="alert error">{finalBlocked ? 'Temporada finalizada.' : 'Limite diário atingido.'}</div>
        )}

        <div className="jobs-top-filters">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔎 Pesquisar vaga..." />
        ...
        <div className="jobs-selection-summary">
          ...
          <div className="jobs-selection-actions">
            ...
            <button className={`primary-btn ${isDemoBlocked || sendHidden ? 'disabled-off' : ''}`} onClick={sendSelected} disabled={selectedIds.length === 0 || loadingJobs || sendHidden || isDemoBlocked}>
        ...
        <div className="jobs-queue-bar">
          <label className="check-row">
            <input type="checkbox" checked={queueRunning} onChange={e => setQueueRunning(e.target.checked)} /> Processar fila
          </label>

          <label className="check-row">
            <input type="checkbox" checked={fastMode} onChange={e => setFastMode(e.target.checked)} /> Modo teste rápido
          </label>
                ...
                return (
                  <div key={job.id} className={`job-master-item ${selectedJob?.id === job.id ? 'active' : ''}`}>
                    <button
                      ...
                      disabled={sendHidden || alreadySent || alreadyQueued || isDemoBlocked}
                      onClick={e => {
                        e.stopPropagation()
                        toggleSelect(job.id)
                      }}
                    ...
                    <div className="job-master-right">
                      <strong>{job.wage}</strong>
                      <small>{job.available || 0} vaga(s)</small>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="jobs-detail-scroll">
              {selectedJob ? (
                <div className="detail-card detail-card-pt">
                  ...
                  <InfoLine label="Estado" value={selectedJob.state || 'Não informado'} />
                  <InfoLine label="Grupo de randomização" value={selectedJob.randomizationGroup || 'Não informado'} />
                  <InfoLine label="Quantidade de vagas" value={`${selectedJob.available || 0}`} />
                  <InfoLine label="Início" value={selectedJob.startDate || 'Não informado'} />
                  <InfoLine label="Fim" value={selectedJob.endDate || 'Não informado'} />
                  <InfoLine label="Valor por hora" value={selectedJob.wage || 'Não informado'} />
                  <InfoLine label="Telefone" value={selectedJob.phone || 'Não informado'} />
                  <InfoLine label="E-mail empregador" value={selectedJob.contact || 'Não informado'} />
  ...
  return (
    <header className="topbar">
      <div className="topbar-brand">
        <RotatingLogo />
        <div>
          <strong>FUTURE EUA H2B</strong>
          <span>Rumo ao sonho americano</span>
        </div>
      </div>
    ...
    <footer className="global-footer">
      ...
      <div style={{ marginTop: '16px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '13px', opacity: 0.85, marginBottom: '6px' }}>
          📧 Contato:{' '}
          <a href="mailto:magno.elen2023@gmail.com" style={{ color: '#60a5fa', textDecoration: 'none' }}>
            magno.elen2023@gmail.com
          </a>
        </p>
        ...
        <p style={{ fontSize: '13px', opacity: 0.85 }}>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{ color: '#25D366', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px', fontWeight: '600' }}
          >
