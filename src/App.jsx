...
<parameter name="id">app-jsx-fixed</parameter>
<parameter name="type">application/vnd.ant.code</parameter>
<parameter name="language">jsx</parameter>
<parameter name="title">App.jsx atualizado com correções</parameter>
<parameter name="content">import { useEffect, useMemo, useState } from 'react'
...
const saved = useMemo(() => loadData(savedUser?.email), [savedUser])
...
// ===== Acordar o backend Render automaticamente =====
useEffect(() => {
console.log('🔄 Acordando backend...')
fetch(${API_URL}/api/health)
.then(r => r.json())
.then(data => console.log('✅ Backend online:', data.message))
.catch(() => console.log('⚠️ Backend ainda dormindo, vai acordar no primeiro envio'))

text

// Manter o backend acordado: ping a cada 10 minutos
...
// ===== Salvar dados POR USUÁRIO (impede burlar o limite) =====
useEffect(() => {
if (user?.email) {
...
try {
  ...
  // Carrega histórico desse usuário (se existir)
  const userData_saved = loadData(data.email)
  setSentLogs(userData_saved.sentLogs || [])
  setQueue(userData_saved.queue || [])
  setSelectedIds([])
...
try {
  ...
  // ===== CARREGA O HISTÓRICO DESSE USUÁRIO =====
  const userData_saved = loadData(userData.email)
  setSentLogs(userData_saved.sentLogs || [])
  setQueue(userData_saved.queue || [])
  if (userData_saved.selectedSeason) {
    setSelectedSeason(userData_saved.selectedSeason)
  }
...
// ===== LOGOUT — NÃO apaga sentLogs/queue (ficam salvos por usuário) =====
...
setUser(null)
localStorage.removeItem(USER_SESSION_KEY)
// sentLogs e queue continuam salvos no localStorage do usuário,
// impedindo o "truque" de deslogar para ganhar mais envios
}
